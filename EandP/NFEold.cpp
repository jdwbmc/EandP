...
void recognizeUsingWebcam(VideoCapture &videoCapture, CascadeClassifier &faceCascade, CascadeClassifier &eyeCascade1, CascadeClassifier &eyeCascade2)
{
    ...
    while (true) {
        ...
        if (faceRect.width > 0) {
			...
			double largest = 0;
			double largest2 = 0; // Largest ellipse area
			int indx = 0;
			int indx2 = 0; //Index of largest ellipse area
			...
			leftEye = displayedFrame(dispLeftEye);
			rightEye = displayedFrame(dispRightEye);
			cvtColor(leftEye, leftEye, CV_BGR2GRAY);
			cvtColor(rightEye, rightEye, CV_BGR2GRAY);
			imshow("LEGray", leftEye);
			imshow("REGray", rightEye);
			threshold(leftEye, leftEye, 220, 255, THRESH_BINARY);
			threshold(rightEye, rightEye, 220, 255, THRESH_BINARY);
			imshow("LEBin", leftEye);
			imshow("REBin", rightEye);
			// Find all contours
			vector<vector<Point> > contours;
			vector<vector<Point> > contours2;
			findContours(leftEye.clone(), contours, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_NONE);
			findContours(rightEye.clone(), contours2, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_NONE);
			// Fill holes in each contour
			drawContours(leftEye, contours, -1, CV_RGB(255,255,255), -1);
			drawContours(rightEye, contours2, -1, CV_RGB(255,255,255), -1);

			// Modification to look for left eye ellipses
			...
			// Modification to look for right eye ellipses
			...
			for (int i = 0; i < contours2.size(); i++)
			{
			 ...
			 // ellipse
			ellipse( leftEye, minEllipse[indx], CV_RGB(255,0,0), 2, 8 );
			ellipse( rightEye, minEllipse2[indx2], CV_RGB(255,0,0), 2, 8 );
			imshow("Pupil LE",leftEye);
			imshow("Pupil RE",rightEye);
